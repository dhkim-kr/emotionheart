...
def main(args):
    ...
    for i in range(1000000):
        #1. Test 셋 샘플링
        test_indices, test_counts, test_std = get_samples(dataset, total_indicies, num_to_sample_test,
                                                                   args.num_classes)
        ...
        # dev_indices, dev_counts, dev_std = get_samples(dataset, remaining_after_test, num_to_sample_dev, args.num_classes)
        #
        # # 4. Dev 셋을 제외한 나머지 데이터 생성 (이것이 Pretrain 셋이 됨)
        # remaining_after_dev = remove_indices(remaining_after_test, dev_indices)

        # 5. Train 셋 정보 계산
        train_indices, train_counts, train_std = get_samples(dataset, remaining_after_test, num_to_sample_train, args.num_classes)
        ...
        if best_counts[0]> best_cnt : #current_total_std < min_total_std :#and worst1< 40 and worst2 < 40:# and best_counts[0]>200 and best_counts[1]>250 and best_counts[2]>250 and best_counts[3] > 250: #and ratio_std < standard:
